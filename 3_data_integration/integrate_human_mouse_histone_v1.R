...
mouse_correct_obj = CreateSeuratObject(counts = mouse_mat_all,
                                       assay = "OpenChromatin")
if (assay == "methyl") {
        load('E:\\GlobusDownload/Chaoran_Methyl/methy_score/hg38_wgbs_dnase_aligned_DHS_seurat_data.rda')
        wgbs_meta$`Tissue/cell types` = NA
        human_correct_obj = add_metadata(human_correct_obj, bind_rows(exp_meta, wgbs_meta))
        load('E:\\GlobusDownload/Chaoran_Methyl/methy_score/mm10_wgbs_dnase_aligned_DHS_seurat_data.rda')
        wgbs_meta$`Tissue/cell types` = NA
        mouse_correct_obj = add_metadata(mouse_correct_obj, bind_rows(exp_meta, wgbs_meta))
} else {
        human_correct_obj = add_metadata(human_correct_obj, exp_meta)
        mouse_correct_obj = add_metadata(mouse_correct_obj, exp_meta)
}

# too many samples from these two biosamples in human H3K27ac
if (assay == "H3K27ac") {
...
if (assay == "methyl") {
        integrated_list = SplitObject(integrated[, integrated$`Assay title` == "WGBS"], split.by = "organism")
